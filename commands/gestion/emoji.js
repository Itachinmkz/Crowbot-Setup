	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id) || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || perm) {
			...
			if (args[0] === "add") {
				...
				if (!customemoji.id) return message.channel.send(`Je n'ai pas accès à cet emoji.`);
			...
			if (args[0] === "remove") {
				...
				if (!message.guild.emojis.cache.get(emoji.id)) return message.channel.send(`Cet emoji n'est pas sur ce serveur.`);
