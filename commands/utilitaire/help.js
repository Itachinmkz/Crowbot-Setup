...
`)
			...
			if (client.config.owner.includes(message.author.id) || db.get(`ownermd_${client.user.id}_${message.author.id}`) === true || perm) {
				...
				const mods = new Discord.MessageEmbed()
					...
					.setTitle("Liste des commandes par permissions")
					.setTimestamp()
					.setDescription(`*Les commandes publiques sont aussi disponible pour la permissions Mods*
...
`)
			...
			util.setTimestamp()
			util.setDescription("*Les paramètres entre **`<>`** sont obligatoires, alors que les paramètres entre **`[]`** sont facultatifs*")
				if (2 <= perm) util.addField(`\`${prefix}alladmin\``, "Permet de voir tous les administrateurs présents sur le serveur")
				if (2 <= perm) util.addField(`\`${prefix}allbot\``, "Permet de voir tous les bots présents sur le serveur")
				if (2 <= perm) util.addField(`\`${prefix}allbotadmin\``, "Permet de voir tous les bots administrateurs présents sur le serveur")
			util.addField(`\`${prefix}banner [membre]\``, "Permet de voir la bannière d'un utilisateur")
			util.addField(`\`${prefix}channelinfo [salon]\``, "Permet d'avoir des informations sur un salon")
			util.addField(`\`${prefix}help\``, "Permet de voir la liste des commandes du bot")
				if (1 <= perm) util.addField(`\`${prefix}help all\``, "Permet de voir les commandes du bot pour chaque permission")
			...
			mods.setTimestamp()
			mods.setDescription("*Les paramètres entre **`<>`** sont obligatoires, alors que les paramètres entre **`[]`** sont facultatifs*")
			if (4 <= perm) mods.addField(`\`${prefix}addrole <membre> <rôle>\` (*Seulement les rôles sans permissions dangereuses*)`, "Permet de donner un rôle à un membre sur le serveur")
			if (2 <= perm) mods.addField(`\`${prefix}ban <membre> [raison]\``, "Permet de bannir un membre du serveur")
			if (2 <= perm) mods.addField(`\`${prefix}banlist\``, "Permet de voir tous les membres bannis sur le serveur")
			if (3 <= perm) mods.addField(`\`${prefix}clear [membre/message]\``, "Permet de supprimer des messages dans un salon")
			if (4 <= perm) mods.addField(`\`${prefix}derank <membre>\``, "Permet d'enlever tous les rôles d'un membre")
			if (2 <= perm) mods.addField(`\`${prefix}kick <membre> [raison]\``, "Permet d'expulser un membre du serveur")
			if (3 <= perm) mods.addField(`\`${prefix}lock [salon]\``, "Permet de fermer un salon du serveur pour le rôle @everyone")
			if (4 <= perm) mods.addField(`\`${prefix}lock all\``, "Permet de fermer tout les salons du serveur pour le rôle @everyone")
			if (1 <= perm) mods.addField(`\`${prefix}mute <membre> [temps/raison]\``, "Permet de rendre muet un membre sur le serveur")
			if (1 <= perm) mods.addField(`\`${prefix}mutelist\``, "Permet de voir les membres muets en direct")
			...
			if (4 <= perm) mods.addField(`\`${prefix}renew all\``, "Permet de recrée tous les salons du serveur")
			if (2 <= perm) mods.addField(`\`${prefix}unban <membre>\``, "Permet de debannir un membre du serveur")
			if (4 <= perm) mods.addField(`\`${prefix}unban all\``, "Permet de debannir tous les membres du serveur")
			...
			if (4 <= perm) mods.addField(`\`${prefix}unmute all\``, "Permet de demute tous les membres muet sur le serveur")
			...
			gestion.setDescription("*Les paramètres entre **`<>`** sont obligatoires, alors que les paramètres entre **`[]`** sont facultatifs*")
			...
			if (4 <= perm) gestion.addField(`\`${prefix}embed\``, "Permet de créer un embed personalisé")
			if (4 <= perm) gestion.addField(`\`${prefix}emoji <add/remove> <emoji>\``, "Permet d'ajouter ou d'enlever un emoji du serveur")
			if (4 <= perm) gestion.addField(`\`${prefix}giveaway\``, "Permet de créer un giveaway")
			if (4 <= perm) gestion.addField(`\`${prefix}giveaway reroll\``, "Permet de rechoisir le gagnant d'un giveaway")
			...
			if (4 <= perm) gestion.addField(`\`${prefix}massrole <add/remove> <rôle>\``, "Permet d'ajouter ou d'enlever un rôle à tous les membres du serveur")
			if (4 <= perm) gestion.addField(`\`${prefix}perm\``, "Affiche la liste des rôles ayant des permissions sur le bot")
			if (4 <= perm) gestion.addField(`\`${prefix}public <add/clear/list/remove> <add/remove: salon>\``, "Permet de gérer les salons où les commandes publiques sont autorisés")
			...
			if (5 <= perm) bot.addField(`\`${prefix}backup <emoji> <clear/create/list/load/remove>> <2/3/4: code>\``, "Permet de gérer les backups sur le bot")
			...
			if (5 <= perm) bot.addField(`\`${prefix}server <invite/leave/list> <leave/invite: ID>\``, "Permet de gérer les serveurs où se trouve le bot")
